...
(function(e, c) {
	...
	f.prototype.reset_input = function() {
		this.$label.attr({
				...
				.attr("class", this.settings.no_icon).prev("img").remove();
	...
	var a = function() {
		...
		var m = !j ? null : ((l instanceof Array || l instanceof FileList)
				? l
				: this.element.files);
...
(function(a, b) {
	a.fn.ace_spinner = function(c) {
		this.each(function() {
			...
			var d = c.max || 999;
			d = ("" + d).length;
			a(this).addClass("spinner-input form-control")
					.wrap('<div class="ace-spinner">');
			var k = a(this).closest(".ace-spinner").spinner(c)
					.wrapInner("<div class='input-group'></div>");
			if (h) {
				a(this)
						.before('<div class="spinner-buttons input-group-btn">							<button type="button" class="btn spinner-down btn-xs '
								+ g
								+ '">								<i class="'
								+ j
								+ '"></i>							</button>						</div>')
						.after('<div class="spinner-buttons input-group-btn">							<button type="button" class="btn spinner-up btn-xs '
								+ e
								+ '">								<i class="'
								+ f
								+ '"></i>							</button>						</div>');
			...
			a(this).on("mousewheel DOMMouseScroll", function(l) {
				var m = l.originalEvent.detail < 0
						|| l.originalEvent.wheelDelta > 0 ? 1 : -1;
				k.spinner("step", m > 0);
			...
			a(this)
					...
					.each(function() {
						...
						e += '<li><a class="' + h
								...
								+ this.value + ';" data-color="' + this.value
								+ '"></a></li>'
					})
					.end()
					.on("change.ace_inner_call", function() {
						a(this).next().find(".btn-colorpicker").css(
								"background-color", this.value)
					})
					.after('<div class="dropdown dropdown-colorpicker"><a data-toggle="dropdown" class="dropdown-toggle" href="#"><span class="btn-colorpicker" style="background-color:'
							+ f
							...
							+ '">'
							+ e
							+ "</ul></div>").next().find(".dropdown-menu").on(
		...
		this.each(function() {
			...
			e
					.html('<div class = "tree-folder" style="display:none;">				<div class="tree-folder-header">					<i class="'
							...
							+ (c["unselected-icon"] == null ? "" : '<i class="'
									+ c["unselected-icon"] + '"></i>')
							+ '				<div class="tree-item-name"></div>			</div>');
			e.addClass(c.selectable == true
					? "tree-selectable"
					: "tree-unselectable");
			e.tree(c)
		});
	...
	a.fn.ace_wysiwyg = function(c, h) {
		...
		this.each(function() {
					...
					switch (p.name) {
						...
						case "viewSource" :
							r += ' <a class="btn btn-sm ' + q
									+ '" data-view="source" title="' + p.title
									+ '"><i class="' + p.icon + '"></i></a> ';
							break;
						default :
							r += ' <a class="btn btn-sm ' + q + '" data-edit="'
									+ p.name + '" title="' + p.title
									+ '"><i class="' + p.icon + '"></i></a> ';
							break
					}
				}
			...
			r.find(".dropdown-menu input:not([type=file])").on(ace.click_event,
				...
				a(this).closest(".dropdown-menu").siblings(".dropdown-toggle")
						.dropdown("toggle")
			...
			r.find(".wysiwyg_colorpicker").each(function() {
				a(this).ace_colorpicker({
							...
							a(this).nextAll("input").eq(0).val(this.value)
									.change()
